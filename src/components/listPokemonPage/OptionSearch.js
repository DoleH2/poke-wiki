...
const OptionSearch = ({ dataPokemon }) => {
    const navigate = useNavigate();
    const { data, error, status } = useGetDetailPokemonQuery({ api: dataPokemon.url });
    return (
        <>
            {status === "fulfilled" ? (
                <div className="frame-option-search d-flex align-items-center" style={{ cursor: 'pointer' }} onClick={() => changeRouter(navigate, "/info", data)}>
